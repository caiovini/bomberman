    ...
    def __init__(self):
        ...
        for x, row in enumerate(materials):
            for i, column in enumerate(row):
                if x == 0:
                    column = pg.image.load(
                        join(path_bomberman, "Front", "Bman_F_f0" + str(i) + ".png")).convert_alpha()
                    materials[x][i] = pg.transform.scale(column, (40, 40))
                elif x == 1:
                    column = pg.image.load(
                        join(path_bomberman, "Back", "Bman_B_f0" + str(i) + ".png")).convert_alpha()
                    materials[x][i] = pg.transform.scale(column, (40, 40))
                elif x == 2:
                    column = pg.image.load(
                        join(path_bomberman, "Side", "Bman_F_f0" + str(i) + ".png")).convert_alpha()
                    materials[x][i] = pg.transform.scale(column, (40, 40))
                elif x == 3:
                    column = pg.image.load(
                        join(path_bomberman, "Side", "Bman_F_f0" + str(i) + ".png")).convert_alpha()
    ...
    def __init__(self):
        ...
        for x, row in enumerate(materials):
            for i, column in enumerate(row):
                if x == 0:
                    column = pg.image.load(
                        join(path_creep, "Front", "Creep_F_f0" + str(i) + ".png")).convert_alpha()
                    materials[x][i] = pg.transform.scale(column, (55, 55))
                elif x == 1:
                    column = pg.image.load(
                        join(path_creep, "Back", "Creep_B_f0" + str(i) + ".png")).convert_alpha()
                    materials[x][i] = pg.transform.scale(column, (55, 55))
                elif x == 2:
                    column = pg.image.load(
                        join(path_creep, "Side", "Creep_S_f0" + str(i) + ".png")).convert_alpha()
                    materials[x][i] = pg.transform.scale(column, (55, 55))
                elif x == 3:
                    column = pg.image.load(
                        join(path_creep, "Side", "Creep_S_f0" + str(i) + ".png")).convert_alpha()
